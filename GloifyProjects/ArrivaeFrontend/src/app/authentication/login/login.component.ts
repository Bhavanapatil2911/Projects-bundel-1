...
export class LoginComponent extends Angular2TokenService implements OnInit {
  ...
  closeModalValidation(){
    return (environment.pptUrl == 'uat' || environment.pptUrl == 'dev')
  }
      ...
      this.authService.logIn(value.email, value.password).subscribe(
        (res) => {
          if (res.status == 200) {
            ...
            this.headers.append(
              "client",
              this.tokenService.currentAuthData.client
            );
            this.headers.append(
              "access-token",
              this.tokenService.currentAuthData.accessToken
            );
            this.customoptions = new RequestOptions({ headers: this.headers });
            // this.loaderService.display(false);
            this.httpCustom
              .get(this.roleUrl, this.customoptions)
              .map((response) => {
                this.currentUserDetails = response.json();
                Object.keys(this.currentUserDetails).map((key) => {
                  this.currentUserDetails = this.currentUserDetails[key];
                });
                ...
                localStorage.setItem(
                  "contact",
                  this.currentUserDetails.contact
                );
                localStorage.setItem(
                  "user_level",
                  this.currentUserDetails.user_level
                );
                localStorage.setItem(
                  "isChampion",
                  this.currentUserDetails.is_champion
                );
                localStorage.setItem(
                  "currentUserName",
                ...
                if (this.currentUserDetails.roles[0] === "designer") {
  ...
  showOTPField() {
    this.authService
      .otpLogin(this.otpLoginForm.controls["email"].value)
      .subscribe(
        (result) => {
          this.loaderService.display(false);
          this.successalert = true;
  ...
  confirmOTP() {
    this.authService
      .confirmOTP(
        ...
        this.otpLoginForm.controls["otp"].value
      )
      .subscribe(
        (result) => {
          this.loaderService.display(false);
  ...
  getToken(result) {
    ...
    this.customoptions = new RequestOptions({ headers: this.headers });
    this.httpCustom
      .get("http://localhost:3000/auth/validate_token", this.customoptions)
      .map((response) => {
        this.authService.redirectAfterLogin.bind(this.authService),
          this.afterFailedLogin.bind(this);
      })
