...
export class AppComponent {
  ...
  ngOnInit() {
    ...
    console.log(this.swUpdate,this.swUpdate.isEnabled,'autoUpdate')
    if (this.swUpdate.isEnabled) {
      ...
      // Listen for the new version being activated
      this.swUpdate.activated.subscribe(event => {
          // `event` will contain information about the previous and current versions
    ...
    this.url = location.origin;
     $("#value1").css("width", "14rem");
     $("#value2").css("width", "calc(100% - 13rem)"); 

    if (!this.isLoggedIn()) {
      localStorage.clear();
      if (
        window.location.pathname === "/reset-password" ||
        window.location.pathname === "/lead/pdf-viewer" ||
  ...
  changeLayout(e) {
    ...
    if(e == 'true'){
      $("#value1").css("width", "6rem");
      $("#value2").css("width", "calc(100% - 5rem)");
