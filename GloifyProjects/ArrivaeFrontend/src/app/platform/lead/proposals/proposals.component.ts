...
@Component({ 
  ...
  providers: [LeadService, QuotationService, DesignerService]
})
export class ProposalsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      if(params['tab'] === 'final_design'){
        this.proposal_type = 'final_design'
      }
      else{
  ...
  changeProposalType(event) {
    this.intialdesign=!this.intialdesign
    this.proposal_type = event;
    this.getProposalList('all');
  }
  ...
  getProposalList(status) {
    ...
    if (this.proposalstatus == 'discount-proposed') {
      this.loaderService.display(true);
      this.quotationService.getDiscountProposedProposalList(this.proposal_type,this.project_id).subscribe(
        res => {
          this.proposalList = res['proposals'];
          this.loaderService.display(false);
        }
  ...
  selectBoq(checked, boqvalue, index) {
    ...
    for (var i = 0; i < this.boqProducts.length; i++) {
      this.final_amt[i] = this.boqProducts[i].total_amount;
      this.pm_fee[i] = this.boqProducts[i].pm_fee;
    }
    ...
    this.quotationService.postProposal(data).subscribe(
      ...
      res => {
        ...
        setTimeout(
          ...
          this.successalert = true;
          this.successMessage ="Proposal Saved Successfully";
          this.loaderService.display(false);
          setTimeout(
            function () {
  ...
  onInputDiscount(elemid, product, index, id: number) {
    ...
    final = final - product.promo_value_applied;
    // ------------claculate PM fee-----------
    if (product.pm_fee > 0) {
      let pm_fee = (final * 8) / 100
      this.pm_fee[index] = pm_fee;

      //-------------claculate Final price-------
      final = final + (final * 8) / 100
  ...
  onInputDiscount_duplicate(elemid, product, index, id: number) {
