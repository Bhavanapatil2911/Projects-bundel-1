...
export class CreateModularQuoteComponent implements OnInit, AfterViewInit {
  ...
  modalExcelForm: FormGroup = new FormGroup({
    name: new FormControl("", Validators.required),
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      this.type_of_boq_created = params["boq_type"];
      this.boq_type = params["boq_state"];
    ...
    if (!this.quotation_id || this.boq_type == "service") {
  ...
  addNestedCustomElems() {
    const getFun = this.customizationModule_MW_Form.get(
      "custom_elements"
    ) as FormArray;
    getFun.push(this.buildCustomElements());
  }
  addAddonForAddons(val, val2?, index?) {
    const getFun = (<FormArray>(
  ...
  getSelectedSections(arg?) {
    ...
    if (this.boqtypeCreation == "create_boq") {
      ...
      if (res != undefined) {
        ...
        this.selectedsectionName = this.selectedSections.includes("others")
          ? "others"
  ...
  getBoqConfig(category) {
    ...
    this.quotationService
      .getBoqGlobalConfig(category, this.quotation_id)
      .subscribe(
        (res) => {
          if (res != null) {
  ...
  createEmptyQuotation() {
    ...
    var data = {
      ...
      quotation: {
        ...
        services: [],
        product_modules: [],
        boq_type: this.boq_type,
      },
    };
    this.quotationService
      .postBOQData(data, this.projectId, this.type_of_boq_created)
      .subscribe(
        (res) => {
          ...
          this.selectedSpacesArrCf = res.quotation.spaces_custom_furniture
            ? res.quotation.spaces_custom_furniture
            : [];
          this.loaderService.display(false);
  ...
  pageChangedOptional(event){
    console.log('clicking optional addons')
    this.current_page2 = event
  ...
  getPricedCustom_elements() {
    this.custom_access = this.selectedSections.includes('loose_furniture') ? false : true;
    this.loaderService.display(true);
    this.quotationService
  ...
  addModule(formval, space, secname, index) {
    if (formval.qty > 0) {
      ...
      if (
            ...
            obj = {
              product_module: {
                ...
                options: {
                  ...
                  shutter_shade_code: !this.boqConfigMWVal
                    .custom_shutter_shade_code
                    ? this.boqConfigMWVal.shutter_shade_code.code
                    : this.boqConfigMWVal.shutter_shade_code,
                  edge_banding_shade_code: !this.boqConfigMWVal
                  ...
                  hardware_brand_id: this.boqConfigMWVal.brand
                    ? this.boqConfigMWVal.brand.id
                    : "",
                  addons: [],
                  custom_elements: [],
                },
            ...
            obj = {
              product_module: {
                ...
                options: {
                  ...
                  shutter_shade_code: !this.boqConfigMKVal
                    .custom_shutter_shade_code
                    ? this.boqConfigMKVal.shutter_shade_code.code
                    : this.boqConfigMKVal.shutter_shade_code,
                  edge_banding_shade_code: !this.boqConfigMKVal
                    .custom_edge_banding_shade_code
                    ? this.boqConfigMKVal.edge_banding_shade_code.code
                    : this.boqConfigMKVal.edge_banding_shade_code,
                  ...
                  skirting_config_height:
                    this.boqConfigMKVal.skirting_config_height,
                  number_exposed_sites: 0,
                  hardware_brand_id: this.boqConfigMKVal.brand
                    ? this.boqConfigMKVal.brand.id
                    : "",
                  addons: [],
                  custom_elements: [],
        ...
        this.loaderService.display(true);
        this.quotationService
          .addModuleToSpace(this.projectId, this.quotation_id, obj)
          .subscribe(
            (res) => {
              this.quotationDetails = res.quotation;
              this.loaderService.display(false);
  ...
  addAppliance(formval, space, secname, index) {
    if (formval.qty > 0) {
      if (this.addApplianceFormMk.valid) {
        ...
        this.loaderService.display(true);
        this.quotationService
          .addApplianceToKitchenSpace(this.projectId, this.quotation_id, obj)
          .subscribe(
            (res) => {
              this.quotationDetails = res.quotation;
              this.loaderService.display(false);
  ...
  addExtra(formval, space, index) {
    if (formval.qty > 0) {
      ...
      this.loaderService.display(true);
      this.quotationService
        .addExtraToKitchenSpace(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
  ...
  addCustomElem(formval, space, secname, index) {
    if (formval.qty > 0) {
      ...
      this.loaderService.display(true);
      this.quotationService
        .addCustomElementToSpace(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            this.loaderService.display(false);
  ...
  updateModule(rowno, mod, secname) {
    ...
    if (this.checkValidationForDimensions(secname, this.addModuleForm)) {
      ...
      this.quotationService
        .updateModuleOfSpace(this.projectId, this.quotation_id, obj2, secname)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
  ...
  updateQuantity(rowno, mod, secname) {
    if (this.editModuleLineQty > 0) {
      ...
      if (mod.job_type === "curtain") {
        ...
        this.quotationService
          .updateQuantity(this.projectId, this.quotation_id, obj2, secname)
          .subscribe(
            (res) => {
              this.quotationDetails = res.quotation;
              this.ref.detectChanges();
  ...
  updateExtra(rowno, mod, secname, combination) {
    if (this.editExtraLineQty > 0) {
      ...
      this.quotationService
        .updateExtraOfSpace(this.projectId, this.quotation_id, obj2)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
  ...
  updateProduct(rowno, mod) {
    if (this.editProductLineItemQty > 0) {
      ...
      if (mod.job_type === "curtain") {
        this.quotationService
          .updateboqJobOfSpaceCurtain(this.projectId, this.quotation_id, objs)
          .subscribe(
            (res) => {
              this.quotationDetails = res.quotation;
              this.editProductLineItemQty = undefined;
        ...
        this.quotationService
          .updateboqJobOfSpace(this.projectId, this.quotation_id, obj2)
          .subscribe(
            (res) => {
              this.quotationDetails = res.quotation;
              this.editProductLineItemQty = undefined;
  ...
  moduleDelete(id, space, category, combined?) {
    if (confirm("Are you sure you want to delete this module?") == true) {
      ...
      this.quotationService
        .deleteModuleToSpace(this.projectId, this.quotation_id, obj, category)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
  ...
  applianceDelete(id, space) {
    if (confirm("Are you sure you want to delete this appliance?") == true) {
      ...
      this.quotationService
        .deleteApplianceToSpace(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
  ...
  extraDelete(id, space) {
    if (confirm("Are you sure you want to delete this extra?") == true) {
      ...
      this.quotationService
        .deleteExtraToSpace(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
  ...
  customelementDelete(id, space) {
    if (
      ...
      this.quotationService
        .deleteCustomElemToSpace(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
  ...
  spaceDelete(space, category) {
    if (confirm("Are you sure you want to delete this space?") == true) {
      ...
      this.quotationService
        .deleteSpaceFromBoq(this.projectId, this.quotation_id, space, category)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            this.selectedSpacesArr = res.quotation.spaces;
  ...
  deleteQuotationOnBackClick(arg) {
    if (this.quotation_id) {
      this.loaderService.display(true);
      this.quotationService
        .deleteQuotation(this.projectId, this.quotation_id)
        .subscribe(
          (res) => {
            if (arg == "boqBack") {
              this.location.back();
  ...
  viewQuotationDetails() {
    ...
    this.loaderService.display(true);
    this.quotationService
      .viewQuotationDetails(this.projectId, this.quotation_id)
      .subscribe(
        (res) => {
  ...
  getCustomizationOfModule(modId) {
    ...
    this.loaderService.display(true);
    this.quotationService
      .getCustomizationOfModule(modId, this.projectId, this.quotation_id)
      .subscribe(
        (res) => {
  ...
  updateCustomization(formval, secname) {
    if (secname == "wardrobe") {
      ...
      this.quotationService
        .updateModuleOfSpace(this.projectId, this.quotation_id, obj3, secname)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            this.ref.detectChanges();
  ...
  saveQuotation(status) {
    ...
    this.quotationService
      .updateBOQData(this.quotation_id, this.projectId, data)
      .subscribe(
        (res) => {
  ...
  uploadKdMaxExcel(space, category, lineno) {
    ...
    this.loaderService.display(true);
    this.quotationService
      .uploadKDMaxExcel(this.projectId, this.quotation_id, data)
      .subscribe(
        (res) => {
          this.quotationDetails = res.quotation;
  ...
  uploadCustomFurnitureExl(event, index, space) {
    ...
    //       this.upload_excel.push(base_encoded_file);
    //     };
  ...
  combineModules(formval) {
    this.loaderService.display(true);
    this.quotationService
      .combineModules(this.projectId, this.quotation_id, formval)
      .subscribe(
        (res) => {
          this.quotationDetails = res.quotation;
  ...
  editcombineModules(formval, modjobid) {
    this.loaderService.display(true);
    this.quotationService
      .editCombinedModule(this.projectId, this.quotation_id, formval, modjobid)
      .subscribe(
        (res) => {
          this.quotationDetails = res.quotation;
          this.combinedModulesForm.reset();
  ...
  addService(form, space, k3) {
    ...
    if (service_obj.quantity > 0) {
      ...
      this.quotationService
        .updateServiceData(this.quotation_id, this.projectId, data)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            this.boqServices_all = [];
  ...
  updateService(i) {
    ...
    for (let j = 0; j < this.servicebojs_boq.length; j++) {
      if (this.servicebojs_boq[j].id == i) {
        if (this.editServciceLineQty > 0) {
          for (let m = 0; m < this.servicebojs_boq.length; m++) {
            if (this.servicebojs_boq[m].job_type === "service_package") {
              this.servicebojs_boq.splice(m, 1);
            }
          }
    ...
    this.quotationService
      .updateServiceJob(this.quotation_id, this.projectId, data)
      .subscribe(
        (res) => {
          this.quotationDetails = res.quotation;
          this.boqServices_all = [];
  ...
  addProductToBoqs2(product, quantity) {
    if (quantity.productQty > 0) {
      ...
      this.quotationService
        .addboqJobToSpacenova(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            // this.generateBoqProductArr();
  ...
  removeProductToBoqs(productid, productname, space) {
    if (confirm("Are you sure you want to delete this product?") == true) {
      ...
      this.quotationService
        .deleteboqJobToSpace(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            // this.generateBoqProductArr();
      ...
      for (var v = 0; v < this.boqProducts.length; v++) {
        if (this.boqProducts[v].space == space) {
          document.getElementById(str).innerText =
            ...
            this.boqProducts[v]["quantity"] *
              this.boqProducts[v]["sale_price"] +
            "";
        }
      }
    }
  ...
  submitdeleteSectionForm(formval) {
    ...
    localStorage.setItem(
      "selected_sections",
      JSON.stringify(this.selectedSections)
    );
    this.quotationService
      .deleteSectionFromBoq(
        this.projectId,
        this.quotation_id,
        deleteCategoryArr
      )
  ...
  servicejobDelete(id) {
    if (confirm("Are you sure you want to delete this service?") == true) {
      ...
      this.quotationService
        .deleteServicejobToSpace(this.projectId, this.quotation_id, obj)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            this.boqServices_all = [];
  ...
  //               }
  //             );
  //         } else if (this.compulsoryaddonsArr.length == 0 && this.pageno_addon < 1) {
  //           this.pageno_addon = 0;
  //           this.quotationService
  //             .getOptionalAddons(
  //               category,
  ...
  savePresetAsGlobal(category, presetId) {
    this.loaderService.display(true);
    this.quotationService
      .savePresetAsGlobal(category, presetId, this.quotation_id)
      .subscribe(
        (res) => {
          this.getBoqConfig(category);
    ...
    if (formval.category == "kitchen") {
      ...
      for (var i = 0; i < keys_arr.length; i++) {
        if (keys_arr[i] == formval.space) {
          for (
            var j = 0;
            j < this.quotationDetails.modular_jobs_kitchen[keys_arr[i]].length;
            j++
          ) {
            mod_job_ids.push(
              this.quotationDetails.modular_jobs_kitchen[keys_arr[i]][j].id
            );
          }
        }
      }
      for (var i = 0; i < keys_arr1.length; i++) {
        if (keys_arr1[i] == formval.space) {
          for (
            var j = 0;
            j < this.quotationDetails.appliance_jobs[keys_arr1[i]].length;
            j++
          ) {
            appliance_job_ids.push(
              this.quotationDetails.appliance_jobs[keys_arr1[i]][j].id
            );
          }
        }
      }
      for (var i = 0; i < keys_arr2.length; i++) {
        if (keys_arr2[i] == formval.space) {
          for (
            var j = 0;
            j < this.quotationDetails.extra_jobs_kitchen[keys_arr2[i]].length;
            j++
          ) {
            extra_job_ids.push(
              this.quotationDetails.extra_jobs_kitchen[keys_arr2[i]][j].id
            );
          }
        }
      }
      ...
      for (var i = 0; i < keys_arr.length; i++) {
        if (keys_arr[i] == formval.space) {
          for (
            var j = 0;
            j < this.quotationDetails.modular_jobs_wardrobe[keys_arr[i]].length;
            j++
          ) {
            mod_job_ids.push(
              this.quotationDetails.modular_jobs_wardrobe[keys_arr[i]][j].id
            );
          }
        }
      }
      for (var i = 0; i < keys_arr2.length; i++) {
        if (keys_arr2[i] == formval.space) {
          for (
            var j = 0;
            j < this.quotationDetails.extra_jobs_wardrobe[keys_arr2[i]].length;
            j++
          ) {
            extra_job_ids.push(
              this.quotationDetails.extra_jobs_wardrobe[keys_arr2[i]][j].id
            );
          }
        }
      }
  ...
  generateSmartQuotation(data) {
    this.loaderService.display(true);
    this.quotationService
      .generateSmartQuotation(this.projectId, this.quotation_id, data)
      .subscribe(
        (res) => {
          this.quotationDetails = res.quotation;
  ...
  activateProduct(tab) {
    ...
    if (tab == "product") {
      ...
      console.log(this.all_segment_activated)
  ...
  activateSubCategory(catalogue_type, catalogue_id, event) {
    ...
    console.log("rturrrrrrrrrrrrrrn")
    this.curtainActive = false;
  ...
  GetNovaData(){
    ...
    this.loaderService.display(true);
    this.catalogueService.GetNovaitems(this.current_page,this.quotation_id,this.minimum_price,this.maximum_price,this.minimum_lead_time,this.maximum_lead_time,this.search_string,this.sort_key, this.liked).subscribe(res=>{
      this.headers_res = res.headers._headers;
      this.per_page = this.headers_res.get("x-per-page");
      this.total_page = this.headers_res.get("x-total");
      this.current_page = this.headers_res.get("x-page");
...
IsFilter = false
  ...
  setCombinationElement(event, idName, elementIndex, addOnsLength, addonIndex) {
    ...
    if ($(`#${idName}-${addonIndex}-${elementIndex}`)) {
      ...
      $(`#${idName}-${addonIndex}-${elementIndex}`)
        .prop("checked", true)
        .change();
      this.ref.detectChanges;
    }
  ...
  nextThree() {
    ...
    if (
      this.shadeStartingIndex + 2 >=
      this.globalVarArr.shutter_shade_code.length - 1
    ) {
      this.showRight = false;
    }
  ...
  deleteLabel(labelIndex, id) {
    this.loaderService.display(true);
    this.quotationService
      .deleteLabelOfLineItem(this.projectId, this.quotation_id, id)
      .subscribe(
        (res) => {
          this.successMessageShow("Label deleted successfully!");
          if (this.selectedsectionName == "modular_kitchen") {
            if (this.ownerableType == "ModularJob") {
              this.quotationDetails.modular_jobs_kitchen[this.labelKey][
                this.indexofType
            ...
            if (this.ownerableType == "ApplianceJob") {
              this.quotationDetails.appliance_jobs[this.labelKey][
                this.indexofType
            ...
            if (this.ownerableType == "ExtraJob") {
              this.quotationDetails.extra_jobs_kitchen[this.labelKey][
                this.indexofType
          ...
          if (this.selectedsectionName == "modular_wardrobe") {
            if (this.isCombined) {
              if (this.ownerableType == "ModularJob") {
                this.quotationDetails.modular_jobs_wardrobe[this.labelKey][
                  this.indexofType
              ...
              if (this.ownerableType == "ModularJob") {
                ...
                this.quotationDetails.extra_jobs_wardrobe[this.labelKey][
                  this.indexofType
                ] = JSON.parse(res._body).extra_job;
                this.labelList =
                  this.quotationDetails.extra_jobs_wardrobe[this.labelKey][
          ...
          if (this.selectedsectionName == "loose_furniture") {
            if (this.ownerableType == "Boqjob") {
              this.quotationDetails.boqjobs[this.labelKey][this.indexofType] =
                JSON.parse(res._body).boqjob;
              this.labelList =
                this.quotationDetails.boqjobs[this.labelKey][
                  this.indexofType
                ].labels;
            }
          }
          if (this.selectedsectionName == "custom_elements") {
            if (this.ownerableType == "CustomJob") {
              this.quotationDetails.custom_jobs[this.labelKey][
                this.indexofType
          ...
          if (this.selectedsectionName == "custom_furniture") {
            if (this.ownerableType == "ShangpinJob") {
              if (this.shangpinType == "door") {
                this.quotationDetails.shangpin_jobs_door[this.labelKey][
                  this.indexofType
              ...
              if (this.shangpinType == "cabinet") {
                this.quotationDetails.shangpin_jobs_cabinet[this.labelKey][
                  this.indexofType
                ] = JSON.parse(res._body).shangpin_job;
                this.labelList =
                  this.quotationDetails.shangpin_jobs_cabinet[this.labelKey][
              ...
              if (this.shangpinType == "accessory") {
                this.quotationDetails.shangpin_jobs_accessory[this.labelKey][
                  this.indexofType
                ] = JSON.parse(res._body).shangpin_job;
                this.labelList =
                  this.quotationDetails.shangpin_jobs_accessory[this.labelKey][
              ...
              if (this.shangpinType == "slidingDoor") {
                this.quotationDetails.shangpin_jobs_sliding_door[this.labelKey][
                  this.indexofType
                ] = JSON.parse(res._body).shangpin_job;
                this.labelList =
                  this.quotationDetails.shangpin_jobs_sliding_door[
                    this.labelKey
                  ][this.indexofType].labels;
              }
              if (this.shangpinType == "wardrobe") {
                this.quotationDetails.shangpin_jobs_wardrobe[this.labelKey][
                  this.indexofType
                ] = JSON.parse(res._body).shangpin_job;
                this.labelList =
                  this.quotationDetails.shangpin_jobs_wardrobe[this.labelKey][
  ...
  editLabel(labelIndex, labelName, id) {
    this.loaderService.display(true);
    this.quotationService
      .editLabelOfLineItem(this.projectId, this.quotation_id, id, labelName)
      .subscribe(
        (res) => {
  ...
  getPromoCode(currentPage, value, categorywisefilter, ofeeringfilter, per_page) {
    ...
    this.loaderService.display(true);
    this.leadService
      .getPromoPackages(currentPage, encodedVal, this.quotation_id, "active",  this.categorywisefilter, this.offeringsfilter , this.promo_per_page)
      .subscribe(
        (res) => {
          this.loaderService.display(false);
          this.isLoading2=false
          // this.promo_headers_res = res.headers._headers;
          // this.promo_per_page = this.promo_headers_res.get("x-per-page");
          // this.promo_total_page = this.promo_headers_res.get("x-total");
          // this.promo_current_page = this.promo_headers_res.get("x-page");
  ...
  addPromoCode(promoCodeId) {
    this.loaderService.display(true);
    this.leadService
      .addPromoPackages(this.projectId, this.quotation_id, promoCodeId)
      .subscribe(
        (res) => {
          this.quotationDetails = res.quotation;
          this.added_promocode_list = res.quotation.promo_jobs;
  ...
  deletePromoCode(promoCodeId) {
    this.loaderService.display(true);
    this.leadService
      .deletePromoPackages(this.projectId, this.quotation_id, promoCodeId)
      .subscribe(
        (res) => {
          this.quotationDetails = res.quotation;
          this.added_promocode_list = res.quotation.promo_jobs;
  ...
  servicePackageDelete(id) {
    if (confirm("Are you sure you want to delete this service?") == true) {
      ...
      this.emptyServiceArray();
      this.generalManagerService
        .deleteServicePackageToSpace(this.projectId, this.quotation_id, id)
        .subscribe(
          (res) => {
            this.quotationDetails = res.quotation;
            this.boqServices_all = [];
  ...
  searchPackage() {
    ...
    this.getServicePackages(1, this.searched_word, "", "");
  }
  ...
  uploadMTOExcelData(value) {
    console.log(this.modalExcelForm.controls);
    this.isLoading = true;
    const data = {
    ...
    this.loaderService.display(true)
    this.quotationService
      .uploadMTOExcelData(this.projectId, this.quotation_id, data)
      .subscribe(
        (res) => {
  ...
  //    this.loaderService.display(true)
  //    this.quotationService.getDisplaymtojobs(this.projectId,this.quotation_id,id,name).subscribe(res=>{
  ...
  DisplaycountForFilters(category , offerdata){
    ...
    this.leadService.getPromocountPackages('active','',this.quotation_id, category,offerdata).subscribe(
      (res) => {
        res = res.json()
        console.log(res);
