...
export class QuotationService {
  ...
  updateBoqSuccessMessage =
    "The Updated BOQ has been added in the Handover / Feasibility Screen. Please click on Handover Button to Handover the BOQ for Design QC and Approval";
  ...
  gettheoptions(sheet_type){
    let url = environment.apiBaseUrl + "/v1/quotations/get_display_names?sheet_type="+sheet_type;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  sendsms(id: Number, status) {
    let url =
      environment.apiBaseUrl +
      "/v1//lead_payment_requests/" +
      id +
      "/send_cashfree_sms";
  ...
  getProductForThisProject(projectId) {
    let url =
      environment.apiBaseUrl + "/v1/projects/" + projectId + "/product-catalog";
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }
  postBOQData(param: any, projectId, typeOfBOQCreated) {
    if (typeOfBOQCreated == "initial") {
      let url =
        environment.apiBaseUrl + "/v1/projects/" + projectId + "/quotations";
      return this.http
        .post(url, param, this.options)
        .map(this.extractData)
        .catch(this.handleErrorObservable);
    }
    if (typeOfBOQCreated == "final") {
      let url =
        environment.apiBaseUrl +
        "/v1/projects/" +
        projectId +
        "/quotations/create_final_boq";
      return this.http
        .post(url, param, this.options)
        .map(this.extractData)
        .catch(this.handleErrorObservable);
  ...
  getQuotationsList(projectId, type,stage,state,qcstate,customerstate,paymentstate) {
    let url =
      environment.apiBaseUrl + "/v1/projects/" + projectId + "/quotations?boq_type="+type+"&boq_stage="+stage+"&boq_status="+state+"&qc_status="+qcstate+"&customer_status="+customerstate+"&payment_status="+paymentstate;
    var params: URLSearchParams = new URLSearchParams();
    var opt = this.options;
    opt.search = params;
    return this.http
  ...
  getAIDeRedirectUrl(quotationId) {
    let url =
      environment.apiBaseUrl + "/v1/quotations/generate_aide_url?quotation_id=" + quotationId;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  viewImportQuotationDetails(projectId, id: number, isFinalBoq?, state?) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
      "/quotations/" +
      id +
      "/import_qoutation?direct_final_boq=" +
      isFinalBoq +
      "&boq_type=" +
      state;
    return this.http
  ...
  viewImportPortfolioDetails(projectId, id, isFinalBoq?, state?) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
      "/quotations/import_portfolio?portfolio_id=" +
      id +
      "&direct_final_boq=" +
      isFinalBoq +
      "&boq_type=" +
    ...
    var obj = {
      portfolio_ids: id.join(','),
      direct_final_boq: isFinalBoq,
      boq_type: state,
    };
    return this.http
  ...
  checkBoqcancreate(projectId) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/can_boq_create?project_id=" +
      projectId;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  quotationApproval(id: Number, params: string) {
    let url =
      environment.apiBaseUrl + "/v1/quotations/" + id + "/approve_quotation";
    let obj = {
      quotation: {
        id: id,
        status: params + "",
  ...
  getDraftProposalList(proposal, projectId, Option) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/drafted_or_saved_proposals?proposal_type=" +
      proposal +
      "&project_id=" +
      projectId +
      "&is_draft=" +
  ...
  getsubmittedProposalList(proposal, projectId) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/submitted_proposals?proposal_type=" +
      proposal +
      "&project_id=" +
      projectId;
    return this.http
  ...
  getGlobalVariable(category, arrivaeSelectValues = false) {
    ...
    if (arrivaeSelectValues)
      url =
        environment.apiBaseUrl +
        "/v1/boq_global_configs?category=" +
        category +
        "&arrivae_select=" +
        arrivaeSelectValues;
    else
      url =
        environment.apiBaseUrl + "/v1/boq_global_configs?category=" + category;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  listofCoreMaterial(
    ...
    if (arrivaeSelectValues)
      url =
        environment.apiBaseUrl +
        "/v1/core_materials/list_shutter_materials?core_material_name=" +
        Core_material_name +
        "&arrivae_select=" +
        arrivaeSelectValues +
        "&category=" +
  ...
  listofShutterFinishes(
    ...
    if (arrivaeSelectValues)
      url =
        environment.apiBaseUrl +
        "/v1/core_materials/list_finishes?shutter_material_name=" +
        shutter_material_name +
        "&arrivae_select=" +
        arrivaeSelectValues +
        "&category=" +
  ...
  listofcorematerial(core, arrivaeSelectValues = false) {
    ...
    if (arrivaeSelectValues)
      url =
        environment.apiBaseUrl +
        "/v1/core_materials/:id/list_shutter_material=" +
        core +
        "&arrivae_select=" +
        arrivaeSelectValues;
    else
  ...
  listofShutterFinishShades(shutter_finish_id, arrivaeSelectValues = false) {
    let url;
    let encodedShutterFinishValue = encodeURI(shutter_finish_id);
    encodedShutterFinishValue = encodedShutterFinishValue.replace(
      /\+/gi,
      "%2B"
    );
    if (arrivaeSelectValues)
      url =
        environment.apiBaseUrl +
        "/v1/shutter_finishes/list_shades?shutter_finish_name=" +
        encodedShutterFinishValue +
        "&arrivae_select=" +
        arrivaeSelectValues;
    else
      url =
  ...
  listofSkirtingConfigs(skirting_type, arrivaeSelectValues = false) {
    ...
    if (arrivaeSelectValues)
      url =
        environment.apiBaseUrl +
        "/v1/skirting_configs/list_configs?skirting_type=" +
        skirting_type +
        "&arrivae_select=" +
        arrivaeSelectValues;
    else
  ...
  getBoqGlobalConfig(category, quotationId) {
    let url =
      environment.apiBaseUrl +
      "/v1/boq_global_configs/get_config?category=" +
      category +
      "&quotation_id=" +
      quotationId;
    return this.http
  ...
  getModulesListOfType(moduletypename, category) {
    let url =
      environment.apiBaseUrl +
      "/v1/module_types/list_modules?category=" +
      category +
      "&module_type_name=" +
      moduletypename;
    return this.http
  ...
  updateQuantity(projectId, quotID, data, category) {
    let url =
      ...
      "/quotations/" +
      quotID +
      "/edit_modular_job_quantity?category=" +
      category;
  ...
  getModuleTypeListOfKitchenCat(kitchenCatID, category, civilkitchen?) {
    let url =
      environment.apiBaseUrl +
      "/v1/kitchen_categories/" +
      kitchenCatID +
      "/kitchen_categories_module_type?civil_kitchen=" +
      civilkitchen;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  getBoqList(proposal, project) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/get_ownerables_for_proposals?project_id=" +
      project +
      "&proposal_type=" +
      proposal;
    return this.http
  ...
  getOBdiscount(value) {
    let url = environment.apiBaseUrl + "/v1/projects/"+value+"/quotations/proposal_ob_boqs";
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getDiscountBoqList(projectId) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/discount_proposed_boq?project_id=" +
      projectId;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  approveDiscountProposal(data, status, propDocId) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/aprove_or_reject_or_change_discount?proposal_doc_id=" +
      propDocId +
      "&discount_status=" +
      status;
    return this.http
  ...
  editCombinedModule(projectId, quotID, data, modjobid) {
    let url =
      ...
      "/quotations/" +
      quotID +
      "/edit_combined_module?modular_job_id=" +
      modjobid;
    return this.http
      .patch(url, data, this.options)
      .map(this.extractData)
  ...
  getAllBoqList(status, proposalId) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/approved_or_rejected_boqs?ownerable_type=" +
      status +
      "&proposal_id=" +
      proposalId;
    return this.http
  ...
  getBoqListByStatus(status, proposalId, Type) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/approved_or_rejected_boqs?is_approved=" +
      status +
      "&proposal_id=" +
      proposalId +
      "&ownerable_type=" +
  ...
  getDiscountProposedProposalList(proposal_type, projectId) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/discount_proposed_boq?proposal_type="+proposal_type+"&project_id=" +
      projectId;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  preclouserpayment(projectId, obj){
    let url = environment.apiBaseUrl + "/v1/projects/" + projectId + "/payments/map_pre_closure_payment";
    return this.http
    .post(url, obj, this.options)
    .map(this.extractData)
    .catch(this.handleErrorObservable);
  ...
  sendPayment(obj) {
    let url =
      environment.apiBaseUrl +
      "/v1/lead_payment_requests/send_cashfree_payment_request";
  ...
  AdjustPayment(project_id , obj){
    // http://localhost:3000/v1/projects/34787/payments/adjust_payment?lead_id=227599&quotations_ids=[46476,46488]&percent=10&amount=3160&stage=pre_10_percent

    // http://localhost:3000/v1/projects/34787/payments/adjust_payment

    let url = environment.apiBaseUrl   + '/v1/projects/' + project_id + '/payments/adjust_payment' 
    return this.http
    .post(url, obj, this.options)
    .map(this.extractData)
    .catch(this.handleErrorObservable);
  ...
  getDeatailsFromNewserver( data){
    let url =   "https://uatapi.arrivae.com/v1/lead_payment_requests/send_cashfree_payment_request" 
    return this.http
    .post(url, data , this.options)
    .map(this.extractDataPage)
    .catch(this.handleErrorObservable);    
  ...
  paymentHistory(id) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/payment_details_for_boq?quotation_id=" +
      id;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  deleteEditBoq(value) {
    let url =
      environment.apiBaseUrl +
      "/v1/proposals/destroy_proposal_docs?proposal_doc_id=" +
      value;
    return this.http
      .delete(url, this.options)
      .map(this.extractData)
  ...
  getServiceActivity(id = "") {
    ...
    if (id) {
      url =
        environment.apiBaseUrl +
        "/v1/service_activities?service_subcategory_id=" +
        id;
    } else {
  ...
  getKitchen_appliances(module_type_id) {
    var url =
      environment.apiBaseUrl +
      "/v1/kitchen_appliances?module_type_id=" +
      module_type_id;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  getPricedCustom_elements(projectId, boqType,custom_element_access?) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
      "/custom_elements/priced_custom_elements?boq_type=" +
      boqType+"&custom_element_access="+custom_element_access;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getHandleList(shutter_finish, product_module_id?) {
    let url =
      environment.apiBaseUrl +
      "/v1/handles/handle_list?shutter_finish=" +
      shutter_finish +
      "&product_module_id=" +
      product_module_id;
    return this.http
  ...
  getListofAddonsForAddons(
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/addons/optional_addons_for_addons?product_module_id=" +
      product_module_id +
      "&slot_name=" +
      slotname +
      "&search_string=" +
  ...
  getMakeforFilter(type) {
    var url = environment.apiBaseUrl + "/v1/brands?type=addon";
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getclassifications(){
    var url = environment.apiBaseUrl + '/v1/addons/classifications'
      return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getKitchenExtras(filterobj?, category?, search_string?, page? , classification?) {
    var filterstr = "&filter_params=" + JSON.stringify(filterobj);
    //
    let url =
      environment.apiBaseUrl +
      "/v1/addons/extras?search_string=" +
      search_string +
      "&category=" +
      category +
      "&page=" +
  ...
  getCompulsoryAddons(
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/addons/compulsory_addons?product_module_id=" +
      product_module_id +
      "&slot_name=" +
      slot_name +
      "&search_string=" +
  ...
  getOptionalAddons(
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/addons/optional_addons?category=" +
      category +
      "&product_module_id=" +
      product_module_id +
      "&search_string=" +
  ...
  subitBoqRemarkOnhold(projectId, quotationId, remark) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  subitBoqRemarkInactive(projectId, quotationId, obj) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  subitBoqRemarkActive(projectId, quotationId, obj) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  getGlobalPresets(category, page) {
    let url =
      environment.apiBaseUrl +
      "/v1/boq_global_configs/list_presets?page=" +
      page +
      "&category=" +
      category;
    return this.http
  ...
  getCategoriesForLoosefurniture() {
    let url =
      environment.apiBaseUrl +
      "/v1/sections/categories_for_spaces?all_categories=true";
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  addboqJobToSpacenova(projectId, quotID, data) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  updateboqJobOfSpace(projectId, quotID, data) {
    ...
    let url =
      ...
      if(data.child_line_item){
        url =  environment.apiBaseUrl +
        "/v1/projects/" +
        projectId +
        "/quotations/" +
        quotID +
        "/update_child_line_item_job";
      }
  ...
  getImportQuotationList(projectId) {
    var url =
      ...
      "/v1/projects/" +
      projectId +
      "/quotations/quotation_for_import";
    if (this.options.params) {
      this.options.params.delete("status");
    }
    return this.http
  ...
  getImportQuotationListimport(projectId,importval) {
    var url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
      "/quotations/quotation_for_import?import="+importval;
    if (this.options.params) {
      this.options.params.delete("status");
    }
    return this.http
  ...
  getFilteredAppliances(module_type_id, filterobj, search_string, page) {
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/kitchen_appliances/filter_appliances?" +
      filterstr;
    return this.http
      .get(url, this.options)
      .map(this.extractDataPage)
  ...
  getDataForApplianceFilters(id) {
    let url =
      environment.apiBaseUrl +
      "/v1/kitchen_appliances/filter_data/?module_type_id=" +
      id;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  getFilteredHandles(
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/handles/filter_handles?category=" +
      category +
      "&shutter_finish_name=" +
      shutter_finish +
      "&handle_type=" +
  ...
  getDataForHandleFilters(category) {
    let url =
      environment.apiBaseUrl + "/v1/handles/filter_data?category=" + category;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  listSubOptions(mo_id) {
    let url =
      environment.apiBaseUrl +
      "/v1/products/get_product_master_sub_options?master_option_id=" +
      mo_id;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  listCatalogueOptions(so_id) {
    let url =
      environment.apiBaseUrl +
      "/v1/products/get_product_catalogue_options?master_sub_option_id=" +
      so_id;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  listVariations(co_id) {
    let url =
      environment.apiBaseUrl +
      "/v1/products/get_product_variants?catalogue_option_id=" +
      co_id;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
  ...
  addLabelToLineItem(
    ...
    return this.http
      .patch(url, reqParams, this.options)
      .map(this.extractDataPage)
      .catch(this.handleErrorObservable);
  }
  ...
  downloadCustomerPerSqBoqPdf(data, boqId, projectId) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  //to split boq
  splitBoqFunction(projectId, quotationsId, array) {
    var obj = {
      line_items: array,
    };
    let url =
  ...
  updateboqJobOfSpaceCurtain(projectId, quotID, data) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
      "/quotations/" +
      quotID +
      "/update_curtain_boqjob";

    let formdata = new FormData();
    formdata.append("boqjob_id", data.boqjob_id);
    formdata.append("curtain[quantity]", data.quantity);
  ...
  uploadMTOExcel(projectId, data,type) {
    const data1 = {
      attachment: data,
      sheet_type: type
    };
    let url =
      ...
      "/v1/projects/" +
      projectId +
      "/quotations/validate_mto_creation";
    return this.http
      .post(url, data1, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }

  uploadMTOExcelData(projectId, quotation_id, data) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  pushToRestore(projectId, quotation_id, data) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  getDisplayData(projectId, quotation_id) {
    let url =
      environment.apiBaseUrl +
      "/v1/projects/" +
      projectId +
  ...
  getDisplaymtojobs(projectId, quotation_id, id, name) {
    let url =
      environment.apiBaseUrl +
      `/v1/projects/${projectId}/quotations/${quotation_id}/mto_jobs_data?display_id=${id}&name=${name}`;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }
  Deletemtojobs(projectId, quotation_id, id) {
    let url =
      environment.apiBaseUrl +
      `/v1/projects/${projectId}/quotations/${quotation_id}/delete_display?display_id=${id}`;
    return this.http
      .delete(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }
  updateExcelData(projectId, quotation_id, data) {
    let url =
      environment.apiBaseUrl +
      `/v1/projects/${projectId}/quotations/${quotation_id}/update_display?`;
    return this.http
      .patch(url, data, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
