...
export class CreateInvoiceComponent implements OnInit {
  ...
  getslidetails(id, removebutton, item) {
    ...
    this.lineItemKey = Object.keys(this.project_id_Invoice.line_items)
    this.isInvoice=this.lineItemsList;
    this.projectDataInfo = [];
    this.checkedList = [];
    $('#collapseSpaceCardLfs' + this.item1).removeClass('show');
  ...
  obj_array;
  item1;
  checkitemkey(event,k2) {
    $('#collapseSpaceCardLfs' + this.item1).removeClass("show");
    this.item1 = k2;
    this.obj_value = event;
    this.obj_array = this.lineItemsList[this.obj_value];
  }
  getslisSelectVal(){
  ...
  SubmitAddedProduct(value){  //submit added product functionality
    ...
    this.loaderService.display(true);
    this.financeService.SubmitInvoices(this.currentProjectId,this.labelval,this.checkedList).subscribe(
      res=>{
        res= res;
        this.edit=[];
        this.edit.length=0;
  ...
  isAllSelectedVal(){
    ...
    this.masterSelected = this.obj_array.every(function(item:any) {
  ...
  getCheckedItemListlist(){  //select unselect functionality of added line item for create invoice 
    ...
    for (var i = 0; i < this.obj_array.length; i++) {
      if(this.obj_array[i].is_invoice==true){
        this.checkedList.push(this.obj_array[i]);
      }
    } 
  }
  ...
  CheckAllOptions() {
    ...
    if (this.obj_array.every(val => val.is_invoice == true)) {
      this.obj_array.forEach(val => { val.is_invoice = false });
      this.checkedList = [];
    }
    else {
      // this.obj_array.forEach(val => { val.is_invoice = true });
      // this.checkedList.push(val);
      for (var i = 0; i < this.obj_array.length; i++) {
        this.obj_array[i].is_invoice = true;
        this.checkedList.push(this.obj_array[i]);
        }
      }
    }
