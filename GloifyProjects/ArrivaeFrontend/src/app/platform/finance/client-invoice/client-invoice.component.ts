...
export class ClientInvoiceComponent implements OnInit {
  ...
  page_number;
  leadList
  getProjectLineItemsDetails(page,startDate='',endDate='',clientId='',search=''){
  
    this.loaderService.display(true);

    this.financeService.getProjectLineItemsDetails(this.currentProjectStatus,page,startDate,endDate,clientId,search).subscribe(
      res =>{
        ...
        if (this.currentProjectStatus === 'released') {
  ...
  checkAccordionStatus(accordionIndex, numberOfAccordions, lineItem) {
    
    this.lineItemKey = Object.keys(lineItem.project_info.line_items);
    //Toggle accordions
  ...
  searchFilter(term: any) {
    ...
    if (this.searched_value !== '') {
      this.getProjectLineItemsDetails('', '', '', '', this.searched_value);
    }
    else if (this.searched_value === '') {
      this.getProjectLineItemsDetails('', '', '', '', '');
    }
  }
  ...
  item1;
  item2;
  checkitemkey(event, event1, k2, i) {
    $('#collapseSpaceCardLfs'+this.item1+this.item2).removeClass("show");
    this.item1 = k2;
    this.item2 = i
    

    this.obj_value = event;
    // if (this.obj_value === "non_panel") {
    //   $('#collapseSpaceCardLfsnon_panel').addClass("show");

    // }
    // if (this.obj_value === "panel") {
    //   $('#collapseSpaceCardsLfspanel').addClass("show");
    // }
    // if (this.obj_value === "services") {
    //   $('#collapseSpaceCardLfsservices').addClass("show");
    // }
    this.obj_array = event1.project_info.line_items[this.obj_value];
