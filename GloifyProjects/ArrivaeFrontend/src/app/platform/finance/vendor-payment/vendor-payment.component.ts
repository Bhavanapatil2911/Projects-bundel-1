...
export class VendorPaymentComponent implements OnInit {
  ...
  submission() {
    if (this.splits[0].amount == this.balvalue) {
      this.errorMessageShow("Payment should split in at least two times");
    } else {
      this.loaderService.display(true);
      this.financeService.splitRetailPay(this.splits, this.id_value).subscribe(
        (res) => {
  ...
  getFinalRequestVendor(e) {
    ...
    this.loaderService.display(true);
    this.financeService
      .getFinalRequestVendor(this.current_page,this.FromDate,this.ToDate,this.clientId,this.searchValue)
      .subscribe((res) => {
        this.loaderService.display(false);
        ...
        this.current_page = this.headers_res.get("x-page");
        res = res.json();
        this.paymentList = res.retail_payment;
         this.leadList = res.vendors;
  ...
  getcompletedvendor(e) {
    ...
    this.loaderService.display(true);
    this.financeService
      .getFinalRequestVendorcompleted(
        this.current_page,
        this.FromDate,
        this.ToDate,
        this.clientId,
  ...
  rejectPayment() {
    if ($("#remarks").val() != "") {
      ...
      this.financeService
        .RejectPi(this.payment_id, this.payment_val, this.isApprove)
        .subscribe(
          (res) => {
  ...
  downloadvendorReport() {
    ...
    this.financeService.downVendorReport().subscribe(
      (res) => {},
      (err) => {
 ...
 this.getVendorPaymentList(
   ...
   this.ageingVal
    ...
    } else{ if(this.selectedState == 'waiting_for_approval'){
 ...
 if (this.expansion != "" && this.expansion != undefined) {
  ...
  approveAll() {
    ...
    if (this.vendor_idss.length === 0) {
      ...
      this.loaderService.display(true);
      this.categoryService.approveAllpayment(this.final_vandors).subscribe(
        (res) => {
  ...
  rejectAll() {
    ...
    if (this.vendor_idss.length === 0) {
      ...
      this.loaderService.display(true);
      this.categoryService.rejectAllpayment(this.final_vandor).subscribe(
        (res) => {
  ...
  approveAll2() {
    ...
    if (this.vendor_idssReq.length === 0) {
      ...
      this.loaderService.display(true);
      this.categoryService.approveAllpaymentreq(this.final_vandors2).subscribe(
        (res) => {
          this.successMessageShow("Payments Approved Successfully !!");
          this.vendor_idssReq = [];
          this.vendor_idsReq = [];
  ...
  rejectAll2() {
    ...
    if (this.vendor_idssReq.length === 0) {
      ...
      this.loaderService.display(true);
      this.categoryService.rejectAllpayment(this.final_vandor2).subscribe(
        (res) => {
          this.successMessageShow("Payments Rejected Successfully !!");
          this.vendor_idssReq = [];
          this.vendor_idsReq = [];
