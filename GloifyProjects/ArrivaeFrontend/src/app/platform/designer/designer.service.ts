...
export class DesignerService {
    ...
    getWipLeads(designerId,status, page?,search?,columnName?,fromDate?,toDate?,score?,CatBucket?,Catname?,Catvalue?){
      ...
      params.set("category_bucket", CatBucket);
      params.set(Catname,Catvalue );
      this.options.search = params;
      return this.http.get(url,this.options)
      .map(this.extractDataPage)
     .catch(this.handleErrorObservable);
    ...
    resendmagic(id) {
      let url =environment.apiBaseUrl+"/v1/users/customer_generate_magic_link?project_id="+id;
      let data = [];
    ...
    ShareOtp(id,status,Date,extend) {
      if(status== 'active'){
        status = 'un_hold'
      }
      let url;
      ...
      if(status == 'on_hold'){
        console.log(Date.customer_meeting_time,Date['customer_meeting_time'])
        
         url =environment.apiBaseUrl+"/v1/users/share_otp_to_change_customer_status?project_id="+id+"&customer_status="+status+`&customer_meeting_time=${Date.customer_meeting_time}&reason_for_lost=${Date.reason_for_lost}&remarks=${Date.remarks}`;

      } else{
        if(status == 'extend_on_hold'){
          url =environment.apiBaseUrl+"/v1/users/share_otp_to_change_customer_status?project_id="+id+"&customer_status="+status+`&scheduled_at=${extend.scheduled_at}&reason_for_lost=${extend.reason_for_lost}`;
        } else{
          url =environment.apiBaseUrl+"/v1/users/share_otp_to_change_customer_status?project_id="+id+"&customer_status="+status;
        }
    ...
    VerifyOtp(id,otp){
      ...
      let url =environment.apiBaseUrl+"/v1/users/verify_customer_status_otp?project_id="+id+"&otp="+otp;
  ...
  updateCancelledStatus(designerId,remarks,customer_id,project_id){
    let url = this.designerUrl+designerId+'/change_customer_status';
    this.options.search =null;
    console.log(remarks)
    var obj = {
        'customer_status' : 'inactive',
        'lost_reason': remarks.reason_for_drop,
        'reason_lost' :remarks.reason,
        'reason_remark_lost': remarks.reason_remark,
