...
export class CatalogueService {
  ...
  getCatalogueList(): Observable<Catalogue[]> {
    return this.http
      .get(this.catalogueUrl, this.options)
  ...
  getCatalogueServiceList(id: Number): Observable<Catalogue[]> {
    return this.http
      .get(this.catalogueUrl + "?parent_id=" + id, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }
  ...
  createCatalogue(catalogue: Catalogue): Observable<Catalogue> {
    ...
    return this.http
      .post(url, catalogue, this.options)
  ...
  getServices(secid) {
    let headers = this.authService.getHeaders();
    let url =
      this.catalogueUrl + "/" + secid + "/catalogue_services/all_service_list";
    return this.http
      .get(url, headers)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  fetchCategories(space_ids) {
    ...
    let url =
      ...
      "/v1/sections/categories_for_spaces?space_ids=" +
      space_ids;
    return this.http
      .get(url, headers)
      .map(this.extractData)
  ...
  fetchSubCategories(category_ids, space_ids) {
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/sections/configurations?category_ids=" +
      category_ids +
      "&space_ids=" +
      space_ids;
    return this.http
  ...
  filterProducts(
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/sections/products/filter?search_string=" +
      search_string +
      "&filter_params=" +
      filter_params;
    return this.http
  ...
  getCMList(){
    ...
    if ( x == 'area_sales_manager' ){
      area_sales  = '/v1/users/list_cms_leads_for_asm'
    }else {
      area_sales = "/v1/users/list_lead_assigned_cm"
    }
    let url =  environment.apiBaseUrl + area_sales;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  fetchSliderValues(
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/sections/products/slider_ranges?search_string=" +
      search_string +
      "&filter_params=" +
      filter_params;
    return this.http
  ...
  filterNewProducts(
    ...
    let url =
      environment.apiBaseUrl +
      "/v1/sections/products/filter?subcategory_ids=" +
      subcategory_ids +
      "&search_string=" +
      search_string +
      "&filter_params=" +
  ...
  fetchProductDetailsnova(product_id) {
    let url = environment.apiBaseUrl + `/v1/child_line_items/${product_id}/fetch_child_line_item_product_details`;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  ProductDetailsnovalike(product_id) {
    let url = environment.apiBaseUrl + `/v1/child_line_items/${product_id}/like`;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  segmentShow(category_type, category_id, page?) {
    ...
    if (category_type == "segment") {
      url =
        environment.apiBaseUrl +
        "/v1/catalog/segment_show?segment_id=" +
        category_id;
    } else if (category_type == "category") {
      url =
        environment.apiBaseUrl +
        "/v1/catalog/category_show?category_id=" +
        category_id;
    }
    let params: URLSearchParams = new URLSearchParams();
    params.set("page", page);
  ...
  catalogType(userId, value) {
    ...
    let url =
      environment.apiBaseUrl + "/v1/users/" + userId + "/change_catalog_type";
    return this.http
      .post(url, obj, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }
  GetNovaitems(page,qid,minPrice,maxPrice,Mintime,Maxtime,search,sort,liked) {
    page = page?page:''
    this.options.search = null
    let url =
      environment.apiBaseUrl + "/v1/child_line_items/fetch_child_line_item_product?page="+page+"&quotation_id="+qid+`&rate_from=${minPrice}&rate_to=${maxPrice}&minimum_lead_time=${Mintime}&maximum_lead_time=${Maxtime}&search_string=${search}&sort_key=${sort}&liked=${liked}&per_page=12`;
    return this.http
      .get(url, this.options)
      .map(this.extractDataPage)
      .catch(this.handleErrorObservable);
  ...
  getCurtainList(page?, search?, liked?, news?) {
    if (search === undefined) {
      search = "";
    }
    let filter_hash = {};
    filter_hash["new"] = news;
    filter_hash["liked"] = liked;
    let filter_params = JSON.stringify(filter_hash);
    let url =
      environment.apiBaseUrl +
  ...
  getCurtaindetailsById(id) {
    let url = environment.apiBaseUrl + "/v1/curtains/" + id;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getHardwares() {
    let url = environment.apiBaseUrl + "/v1/curtains/hardwares";
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getFabrics() {
    let url = environment.apiBaseUrl + "/v1/curtains/fabrics";
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getVariants(id?) {
    let url = environment.apiBaseUrl + "/v1/curtains/" + id + "/variants";
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getCurtainPrice(curtainId, width, height, fabricId, hardwareId) {
    let url =
      environment.apiBaseUrl +
      "/v1/curtains/" +
      curtainId +
  ...
  addboqJobToSpace(projectId, quotID, data) {
    ...
    formdata.append("curtain[id]", data.id);
    formdata.append("curtain[curtain_fabric_id]", data.fabric_id);
    formdata.append("curtain[space]", data.space);
    formdata.append("curtain[curtain_hardware_id]", data.hardware);
    formdata.append("curtain[quantity]", data.quantity);
    formdata.append("curtain[width]", data.width);
    formdata.append("curtain[height]", data.height);
    return this.http
  ...
  likecurtainProduct(product_id) {
    let url = environment.apiBaseUrl + "/v1/curtains/" + product_id + "/like";
    return this.http
      .post(url, {}, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  ...
  getCMdetailsList(smid){
    let url = environment.apiBaseUrl + "/v1//users/get_cm_pop_up_data?sales_manager_id="+smid;
    return this.http
      .get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
