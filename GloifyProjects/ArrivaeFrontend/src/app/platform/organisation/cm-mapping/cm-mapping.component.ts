...
export class CmMappingComponent implements OnInit {
  ...
  restructureDesignStudio(event,cmId){
    if (event.target.checked) {
      if (confirm("Do you really want to assign this CM as design studio CM ? ")) {
        this.loaderService.display(true);
        this.cmService.assignAsDesignStudio(cmId,'active').subscribe(
          res=>{
          this.loaderService.display(false);
          this.successalert = true;
          this.successMessage = 'Selected community manager assigned as a design studio';
          this.getCmTags();
          this.getAllCmList();
          this.getAllTags();
          this.getRepopulate();
        ...
        $("#box-"+cmId).prop('checked',false);
        this.getCmTags();
        false;
      }
    }else{
      if (confirm("Do you really want to unassigned this CM as design studio CM ?")) {
        this.loaderService.display(true);
        this.cmService.assignAsDesignStudio(cmId,'inactive').subscribe(
          res=>{
          this.loaderService.display(false);
          this.successalert = true;
          this.successMessage = 'Selected community manager unassigned as a design studio';
          this.getCmTags();
          this.getAllCmList();
          this.getAllTags();
          this.getRepopulate();
