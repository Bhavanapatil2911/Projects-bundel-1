import { Component, OnInit,OnDestroy ,AfterViewInit, Input, Type } from '@angular/core';
...
export class CmLeadsListComponent implements OnInit, OnDestroy ,AfterViewInit {
  ...
  sm_list: any;
   ...
   onKey(event: any) { // without type info
    ...
    var  i=0;
    if(true){
      this.getUserList('',this.search_value,this.searchCM);
      i++;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      this.CM = params['searchCM'];
      this.SM = params['searchSM'];
    ...
    createForm(){
      this.statusChangeForm = this.formBuilder.group({
        ...
        reason_for_lost: new FormControl("",Validators.required),
  ...
  getUserList(page?,search?,cm?) {
    this.loaderService.display(true);
    this.cmService.getQualifiedUgetCMListserList(this.CMId,this.lead_status,page,search,this.CM,this.SM,this.designer_id, this.to_date,this.from_date, this.filtercol1Val).subscribe(
      res => {
        ...
        this.getDesignerList(this.CMId);
  ...
  getDesignerList(id?){
    this.cmService.getDesignerList(this.CMId,id).subscribe(
      res => { 
  ...
  onDropdownFocus(id){
    if(this.role=='area_sales_manager' || this.role=='sales_manager'){
      this.designersList=[]
      this.getDesignerList(id)
 ...
 private designerLeadsCount(client_id,id:number,index){
  ...
  reasonForLostDropdownChange(val){
    ...
    submitDropLeadTypeSublost(type){
      this.dropdownDropremark=type;
      this.statusChangeForm.controls['reason_remark'].setValue(type);
    }
  // lost
  submitDropLeadTypelost(type){
    this.dropdownDropType=type;
    this.statusChangeForm.controls['reason_for_lost'].setValue(this.dropdownDropType);
    this.dropdownDropreasonLost = undefined;
    this.statusChangeForm.controls['reason'].setValue("");
    this.statusChangeForm.controls['reason_remark'].setValue("");
    if(type == 'others'){
      this.statusChangeForm.controls['reason_remark'].setErrors(null);
      this.statusChangeForm.controls['reason_remark'].updateValueAndValidity
      }

  }
  submitDropLeadTypeReasonlost(type){
    console.log("jjjj");
    this.dropdownDropreasonLost=type;
    this.dropdownDropremark = undefined;
    this.statusChangeForm.controls['reason'].setValue(type);
    console.log(this.dropLeadForm.controls['reason'].value)
    this.statusChangeForm.controls['reason_remark'].setValue("");
    if(type == 'fhi' || type == 'phi' || type == 'only_on_site-services_execution' || type == 'designer'  || type == 'ec'  || type == 'design'  ){
      this.statusChangeForm.controls['reason_remark'].setValidators(Validators.required)
      this.statusChangeForm.controls['reason_remark'].updateValueAndValidity
    } else{
      this.statusChangeForm.controls['reason_remark'].setErrors(null);
  ...
  onLostSubmit(){
    ...
    this.statusDetails["lost_reason"] = this.statusChangeForm.controls['reason_for_lost'].value
    this.statusDetails["reason_lost"] = this.statusChangeForm.controls['reason'].value
    this.statusDetails["reason_remark_lost"] = this.statusChangeForm.controls['reason_remark'].value
  ...
  reset(){
    $("#cuSelct"+this.ProjectId).prop("selectedIndex",0 );
