...
export class CommunitymanagerService {
  ...
  getQualifiedUgetCMListserList(userId,lead_category,page?,search?,CM?,SM?,designer_id?, to_date?, from_date?, colName?){
    ...
    opt.search = params;
  	let url = this.communitymanagerUrl+userId+'/cm_dashboard'+'?cm_id='+CM+'&sm_id='+SM;
  ...
  getDesignerList(userId,id?){
    id=id?id:userId
    let url = this.communitymanagerUrl+userId+'/cm_designers?cm_id='+id;
  ...
  getFiterListOfCMandDM(cmid?,smid?){
    let url=this.communitymanagerUrl+'list_cm_designer_for_sm'+'?cm_id='+cmid+'&sm_id='+smid;
  ...
  getFiterListOfCMandSM(smid?){
    smid=smid==undefined?'':smid;
    let url=this.communitymanagerUrl+'list_cm_designer_for_sm?sm_id='+smid;
    let params =null
    this.options.search=params
    return this.http.get(url,this.options)
    .map(this.extractData)
  ...
  getFiterListOfCM(cmid?){
    let url=this.communitymanagerUrl+'list_cm_designer_for_sm'+'?cm_id='+cmid;
    return this.http.get(url,this.options)
    .map(this.extractData)
    .catch(this.handleErrorObservable);
  ...
  getFilterListofWIPCM(sm_id){
    sm_id=sm_id==undefined?'':sm_id
    let url=this.communitymanagerUrl+'list_cm_designer_for_sm?sm_id=' + sm_id;
    return this.http.get(url,this.options)
    .map(this.extractData)
    .catch(this.handleErrorObservable);
  }
  ...
  getFilterListofWIPCMForDM(){
    ...
    let url=this.communitymanagerUrl+`list_of_dgm_cm_mapping?search=&dgm_id=${dgm_id}`;
    return this.http.get(url,this.options)
    .map(this.extractData)
    .catch(this.handleErrorObservable);
  }
  ...
  getCmDashboardCount(CMID,date_from,date_to, filter_designer,CM?,DM?,SM?){
    console.log(CM)
    CM=CM==undefined?'':CM
    SM=SM==undefined?'':SM

    let url = this.communitymanagerUrl+CMID+'/cm_dashboard_count'+'?cm_id='+CM+'&sm_id='+SM;
    if(date_from || date_to || filter_designer || CM || DM || SM){
      let params: URLSearchParams = new URLSearchParams();
      if(date_from){
  ...
  getUserCountsByStatusInWip(userId,designer_id,CM,SM){
    ...
    let url = this.communitymanagerUrl+userId+'/cm_wip_dashboard_counts?designer_id=' + designer_id + '&cm_id=' + CM + '&sm_id=' + SM;
    // let params: URLSearchParams = new URLSearchParams();
    // params.set('designer_id', designer_id);
    // params.set('cm_id', CM);
    // params.set('sm_id', SM);
  ...
  getWipList(CMId,status,designerID,page?,search?,CM? , SM?,ppmr?,score?,bu_filter?,catBucket?,catName?,catValue?){
    if(catName == 'ob_bucket'){
      catName = 'ob_category'
    } else{
      catName = 'oc_category'
    }
    let url = this.communitymanagerUrl+CMId+'/cm_wip_leads?page=' + page + 
    '&search=' + search + 
    '&status=' + status + 
    '&cm_id=' +CM + 
    '&sm_id=' + SM + 
    '&price_match_status=' + ppmr + 
    '&designer_id=' + designerID + 
    '&score=' + score + 
    '&bu_head_approval=' + bu_filter +
    "&category_bucket="+catBucket+"&"+catName+"="+catValue
    ...
    // params.set('cm_id',CM)
    // params.set('sm_id',SM)
    // params.set("price_match_status", ppmr);
    // params.set('designer_id', designerID);
    // params.set('score',score)
    // params.set('bu_head_approval',bu_filter)
  ...
  // updateonholdEvent(eventId,eventId2,obj1,obj2){
  //   let eventUrl = environment.apiBaseUrl+'/v1/events/'+eventId+'/reschedule_event?ownerable_type=Project&ownerable_id='+eventId2;
  //   var obj= {
  //     "event":{
  //       "scheduled_at" : obj1,
  //       "remark": obj2,
  ...
  droppedLead(leadId,remarks) {
    ...
    var obj = {
        'lead_status' : 'dropped',
        'drop_reason': remarks.reason_for_drop,
        'reason_drop' :remarks.reason,
        'reason_remark_drop': remarks.reason_remark
    }
...
//get the Intake status of the community manager
  ...
  assignAsDesignStudio(cm_id,status){
    let url = environment.apiBaseUrl + '/v1/users/'+cm_id+'/cm_design_studio_toggle';
    let formdata = new FormData();
    formdata.append('design_studio_status', status);
    return this.http.post(url, formdata, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }

  getLeadNOCHighlights(cmid?,smid?) {
   cmid = cmid?cmid:'';
   smid = smid?smid:'';
    let url = environment.apiBaseUrl + '/v1/lead_nocs/lead_noc_highlights?cm_ids='+cmid+'&cm_id='+cmid+'&sm_id='+smid;
    return this.http.get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }

  getthreeplusmeeting(cmid?,smid?) {
  cmid = cmid?cmid:'';
   smid = smid?smid:''; 
    let url = environment.apiBaseUrl + '/v1/lead_nocs/lead_noc_meeting_leads?cm_ids='+cmid+'&cm_id='+cmid+'&sm_id='+smid;;
    return this.http.get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }
