...
export class BusinessHeadService {
  ...
  DisablePromoPackageExpirydate(projectId, quoteId) {
    ...
    var jsondata = {}
    let apiurl = environment.apiBaseUrl + '/v1/projects/' + projectId + '/quotations/' + quoteId + '/re_apply_all_expired_promo_codes';
    return this.http.post(apiurl, jsondata, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);

  }

  EnablePromoPackageExpirydate(projectId, quoteId) {
    this.headers = new Headers({ 'enctype': 'multipart/form-data' });
    this.headers.append('Accept', 'application/json');
    this.headers.append('uid', this.tokenService.currentAuthData.uid);
    this.headers.append('client', this.tokenService.currentAuthData.client);
    ...
    this.options = new RequestOptions({ headers: this.headers });
    var jsondata = {}
    let apiurl = environment.apiBaseUrl + '/v1/projects/' + projectId + '/quotations/' + quoteId + '/remove_expired_promo_codes';
    return this.http.post(apiurl, jsondata, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
