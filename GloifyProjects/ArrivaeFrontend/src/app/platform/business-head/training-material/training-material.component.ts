...
export class TrainingMaterialComponent implements OnInit {
  ...
  onChange(event) {
    ...
    this.fileDetails = (
      document.getElementById("files") as HTMLInputElement
    ).value;
  }
  UploadDD(event) {
    this.upload_Item = [];
  ...
  upload() {
    this.loaderService.display(true);
    this.businessHeadService
      .uploadMaterial(this.upload_Item, this.selectSubcategory)
      .subscribe(
        (res) => {
          this.loaderService.display(false);
          this.successalert = true;
  ...
  sanurl = "https://view.officeapps.live.com/op/embed.aspx?src=";
  pageNo = 1;
  updatedUrl;
  sanitize(url: string) {
    return this.sanitizer.bypassSecurityTrustResourceUrl(
      url.split("?")[0] + "#toolbar=0&view=Fit"
    );
  }
  sanitize2(url: string) {
    return this.sanitizer.bypassSecurityTrustResourceUrl(this.sanurl + url);
  }
  ...
  ngAfterViewInit() {
    ...
    // const modalContent = this.elementRef.nativeElement.querySelector('.preview');

    // modalContent.addEventListener('keydown', (event) => {
    //   if (event.key === 'ArrowLeft' && this.showPdfLeft) {
    //     this.prevPdfPage();
    //   } else if (event.key === 'ArrowRight' && this.showPdfRight) {
    //     this.nextPdfPage();
    //   } else if (event.key === 'ArrowUp' && this.showPdfLeft) {
    //     this.prevPdfPage();
    //   } else if (event.key === 'ArrowDown' && this.showPdfRight) {
    //     this.nextPdfPage();
    //   }
    // });
  ...
  showModal(FILE_URL, total_page_count) {
    ...
    setTimeout(() => {
      this.updatedUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
        this.file_new_url.split("?")[0] + "#toolbar=0&page=1&view=Fit"
      );
    }, 500);
  }
  ...
  handleKeyboardEvent(event: KeyboardEvent) {
    if ((event.key === 'ArrowLeft' || event.key === 'ArrowUp')&& this.showPdfLeft) {
      this.prevPdfPage();
    } else if ((event.key === 'ArrowRight'|| event.key === 'ArrowDown') && this.showPdfRight) {
      this.nextPdfPage();
    }
  }
  ...
  updatedUrl2:any;
  showModal2(FILE_URL, total_page_count) {
    window.open(this.sanurl+FILE_URL, "_blank");
  }

  image_file_url;
  showimageModal(image_url) {
    $("#poPreviewModalimg").modal("show");
    this.image_file_url = image_url;
  }
  ...
  editsubCategory() {
    this.loaderService.display(true);
    this.businessHeadService
      .editsubCategory(this.subcategoryForm.value, this.subcategoryId)
      .subscribe(
        (res) => {
          this.categoriesList = res.training_materials;
  ...
  nextPdfPage() {
    ...
    setTimeout(() => {
      this.updatedUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
        this.urlWithoutPdfParameters + "#toolbar=0&view=Fit&page=" + this.pageNo
      );
    }, 500);
    this.ref.detectChanges();
  }
