...
export class CalenderService { 
  ...
  fetchEvent(from_date, to_date, actionables_of?,event_type?, gm_id?, cm_id?, designer_id?) {
    if (gm_id === undefined) {
      gm_id=''
    }
    if (cm_id === undefined) {
    ...
    let eventUrl = environment.apiBaseUrl+'/v1/events?actionables_of='+actionables_of+'&from_date='+from_date+'&to_date='+to_date+'&event_type='+event_type+'&gm_ids='+gm_id+'&cm_ids='+cm_id+'&designer_ids='+designer_id;
    return this.http.get(eventUrl, 
            this.options).map((res: Response) => res.json());
  }
  ...
  fetceventTypefilter(from_date,to_date,event_type,search){
    var eventUrl;
    // if(designerId == "all"){
    if (search === undefined) {
      search = ''
    }
      eventUrl = environment.apiBaseUrl+'/v1/events?from_date='+from_date+'&to_date='+to_date+'&event_type='+event_type+'&search='+search;
    // }
    // else{
    //   eventUrl = environment.apiBaseUrl+'/v1/events?designer_id='+designerId+'&from_date='+from_date+'&to_date='+to_date;
    // }
  ...
  fetceventTypefilterlocal(from_date,to_date,event_type,search){
    var eventUrl;
    // if(designerId == "all"){
    if (search === undefined) {
      search = ''
    }
      eventUrl = environment.apiBaseUrl+'/v1/events/events_for_ownerable?from_date='+from_date+'&to_date='+to_date+'&event_type='+event_type+'&search='+search;
    // }
    // else{
    //   eventUrl = environment.apiBaseUrl+'/v1/events?designer_id='+designerId+'&from_date='+from_date+'&to_date='+to_date;
    // }
  ...
  updateStatusDesignEvent(obj ,eventId){    
    let eventUrl = environment.apiBaseUrl+'/v1/events/'+eventId+"/change_event_status";
    return this.http.post(eventUrl, obj, 
    this.options).map((res: Response) => res.json());
  }
  ...
  getListofAgenda() {
    let url = environment.apiBaseUrl + '/v1/mobile/events/get_list_of_agenda';
    return this.http.get(url, this.options)
      .map(this.extractData)
      .catch(this.handleErrorObservable);
  }

  getListofContactType() {
    let url = environment.apiBaseUrl + '/v1/events/get_list_of_contact_type';
      return this.http.get(url, this.options)
        .map(this.extractData)
        .catch(this.handleErrorObservable);
  }
