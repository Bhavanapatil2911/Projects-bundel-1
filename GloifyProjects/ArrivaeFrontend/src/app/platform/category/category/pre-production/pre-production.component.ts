...
export class PreProductionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getBOQList(parseInt(localStorage.getItem('taks_page')),this.searchparam);
  ...
  ngOnDestroy(): void{
    ...
    localStorage.setItem('taks_page','1')
  ...
  toggleRow(row) {
    ...
    if (this.rowSelected === -1) {
      ...
      if (this.rowSelected == row.id) {
        this.rowSelected = -1;
        console.log('2')
        this.getBOQList(this.current_page,this.searchparam)
      }
      else {
        this.rowSelected = row.id
      }
  ...
  getBOQList(page,searchparam){
    ...
    if(page){
      localStorage.setItem('taks_page',page)
    }
    this.loaderService.display(true);
      this.categoryService.getBoqListForSli(page,this.searchparam,this.filterCat,this.isNew).subscribe(
      res => {
        console.log(res);
...
filterCat =''
  ...
  searchCategoryProjects(searchparam,filter){
    this.filterCat = filter
    console.log(filter)
    this.loaderService.display(true);
    if(searchparam != ""){
      this.categoryService.getBoqListForSli('',searchparam,filter,this.isNew).subscribe(
        res=>{
  ...
  ProjectId;
  Boq_id;
  GetDynamicsData(project_id,boq_id){
    this.project_id = project_id;
    this.Boq_id = boq_id;

    this.loaderService.display(true);

    this.categoryService.Dynamics_bomData(project_id,boq_id).subscribe(res=>{
      this.DynamicsData = res;
      if($('#ViewDynamics').hasClass('show') == false){
        $('#ViewDynamics').modal('show')
  ...
  triggerDynamics(task){
    this.loaderService.display(true);

    this.categoryService.Dynamics_Trigger(this.project_id,this.Boq_id,task).subscribe(res=>{

 this.successalert = true;
        this.successMessage = "Dynamics Triggered Successfully";
        setTimeout(function() {
              this.successalert = false;
           }.bind(this), 2000);
      this.GetDynamicsData(this.project_id,this.Boq_id);
