...
export class MaterialTrackingComponent implements OnInit {
  ...
  shippingAddressList = [
    ...
    },
    {
      ...
      contactNumber: "9594126041",
      contactName: "Nikhil Zore",
      address:
        "Arrivae Warehouse, Arrivae Warehouse, Boxmyspace, Bldg no. A-6, Gala no 8 to 11,Harihar Compound, Mankoli Naka,Mumbai Nashik Highway, Bhiwandi 421302",
      default: false,
    },
    {
    ...
    {
      addressName: "Vista Modular System",
      contactNumber: "8053485454",
      contactName: "Pradeep",
      address:
    ...
    {
      addressName: "Krish Furniture Components",
      contactName: "Champak",
      contactNumber: "9008451673",
      address:
    ...
    {
      addressName: "Flora Interior Solutions",
      contactName: "Venugopal ",
      contactNumber: "9035241661",
      address:
        "Flora Interior Solutions, 534B, 8th Main, Peenya 2nd stage, Bangalore - 560058 Landmark - Near Bhatt Hotel.",
    },
    {
      addressName: "Futhura Warehouse",
  ...
  billingAddressList = [
    ...
    {
      addressName: "Aide Technologies Pvt Ltd",
      contactNumber: "",
      contactName: "",
  ...
  onQCschedulesubmit() {
    ...
    if (this.sliSelectedId.length == 0) {
      ...
      if (this.selectedSLI.every((item) => item.qc_date)) {
        this.scheduleQualityControl(
          "rescheduled",
          this.selectedMoment,
          this.sliSelectedId,
          this.scheduleQC.controls["scheduleQCRemark"].value,
          []
        );
      } else {
        this.scheduleQualityControl(
  ...
  selectAllSli(event, index) {
    if (event.target.checked) {
      ...
      $('input[name="sli-checkboxes"]').prop(
        "checked",
        true
      ); /*if select-all checkbox is checked then every sli should be selected*/
    } else {
      this.selectedSLI = [];
      this.sliSelectedId = [];
      this.clubbed_job_elements_ids = {};
      $('input[name="sli-checkboxes"]').prop(
        "checked",
        false
      ); /*if It is unchecked all the slis should be unchecked*/
    }
  }
  ...
  selectSLI(event, index, item) {
    ...
    if (this.selectedSLI.length > 0) {
      ...
      //if all the slis are unchecked the select all checkbox should be unchecked as well.
      this.selectAllCheckbox.patchValue({
        selectAllSli: false,
      });
    }
  ...
  sameBillShipAddressToggle() {
    ...
    if (this.raiseDispatchDateForm.controls["sameAddress"].value) {
      let patchAddressData = {
        shippingAddress:
          this.raiseDispatchDateForm.controls["billingAddress"].value, //fetching values from formcontrol
        shippingContactName:
          this.raiseDispatchDateForm.controls["billingContactName"].value,
        shippingContactNumber:
          this.raiseDispatchDateForm.controls["billingContactNumber"].value,
        selectedSavedShippingAddress: 0,
      };
  ...
  setAddress(event, type) {
    this.raiseDispatchDateForm.patchValue({
      //uncheck the checkbox if any address is changed ;
      sameAddress: false,
    });
    ...
    if (type == "shipping") {
      ...
      let addressData =
        this.shippingAddressList[
          this.raiseDispatchDateForm.controls["selectedSavedShippingAddress"]
            .value
        ];
      ...
      let addressData =
        this.billingAddressList[
          this.raiseDispatchDateForm.controls["selectedSavedBillingAddress"]
            .value
        ];
  ...
  scheduleQualityControl(status, scheduledDate, itemList, remarks, files) {
    ...
    let shouldSchedule = this.selectedSLI.every(
      (sli) => sli.qc_status == statusOfFirstItem
    );
    if (this.sliSelectedId.length > 0) {
      if (shouldSchedule) {
        //if there are more than one distinct status in the sli list  then user is redirected to error message.
        this.loaderService.display(true);
    ...
    if (this.sliSelectedId.length > 0) {
      ...
      if (this.completeQCForm.controls["upload"].value) {
        this.scheduleQualityControl(
          ...
          this.sliSelectedId,
          "",
          this.completeQCForm.controls["upload"].value.fileList
        );
      } else {
        this.scheduleQualityControl(
          "completed",
  ...
  saveDispatchReadiness() {
    if (this.sliSelectedId.length > 0) {
      this.saveDispatchReadinessDate(
        this.sliSelectedId,
        this.selectedMoment,
        this.dispatchReadinessForm.controls["qcReadinessRemark"].value
      );
    } else {
      this.errorMessage = "Please select atleast one SLI .";
      this.errorAlert = true;
  ...
  raiseDispatchDate(dispatchedBy) {
    if (this.sliSelectedId.length > 0) {
      let scheduleDate =
        this.raiseDispatchDateForm.controls["dispatchDate"].value;
      let status = "scheduled";
      let site = this.raiseDispatchDateForm.controls["siteWarehouse"].value;
      let billingAddress =
        this.raiseDispatchDateForm.controls["billingAddress"].value;
      let shippingAddress =
        this.raiseDispatchDateForm.controls["shippingAddress"].value;
    ...
    if (this.sliSelectedId.length > 0) {
      ...
      this.categoryService
        .saveDispatchScheduleDate(
          ...
          "",
          "",
          this.partialDispatchForm.controls["itemDescription"].value,
          this.partialDispatchForm.controls["itemBalance"].value,
          [],
          this.clubbed_job_elements_ids
        )
        .subscribe(
    ...
    if (this.sliSelectedId.length > 0) {
      ...
      this.categoryService
        .deliverItems(
          this.sliSelectedId,
          status,
          this.partialDeliveryForm.controls["itemDescription"].value,
          this.partialDeliveryForm.controls["itemBalance"].value,
          this.clubbed_job_elements_ids
        )
        .subscribe(
          (res) => {
  ...
  onFileChange(event, feature) {
    ...
    if (event.target.files.length > 1) {
      this.fileUploadStatus =
        event.target.files.length + " Files were Uploaded.";
    } else {
  ...
  refreshPOList() {
    this.loaderService.display(true);
    this.categoryService
      .fetchAllPoList(this.quotation_id, this.poobj.id)
      .subscribe(
        (res) => {
  ...
  /* authored by : Kaartik@Gloify.com */
  rescheduleDefaultValues() {
    //default values for rescheduling
    if (this.sliSelectedId.length == 1) {
      //these default values are only neede when the seleted slis count is one.
      this.scheduleQC.patchValue({
        scheduleQCRemark: this.selectedSLI[0].qc_remarks,
      });
      this.selectedMoment =
        new Date(Date.parse(this.selectedSLI[0].qc_date)) || new Date();
    }
  }
  /* authored by : Kaartik@Gloify.com */
  downloadQCReport() {
