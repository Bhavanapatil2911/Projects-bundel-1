...
@Component({
  ...
  providers: [CategoryService, QuotationService, LeadService, FloorplanService],
})
export class AcceptanceComponent implements OnInit {
  ...
  curentdep: any;
  existeddep: any;
  projectHandoverId: any;
  changedep(e) {
    this.curentdep = e.department;
    this.existeddep = e.department;
    this.projectHandoverId = e.id;
  }
  getCategorisedList() {
      ...
      .subscribe(
        (res) => {
          ...
          if (res.line_marking_completed) {
            this.lineMarkingCompleted =
              res.project_handover.line_marking_completed;
          }
          ...
          for (let i = 0; i < this.project_list.all_files.length; i++) {
            if (this.all_file_list.all_files[i] == "Quotation") {
              ...
              this.sortArrayOFItems.push("MTOFiles");
              this.panelDiv = true;
            } else if (this.all_file_list.all_files[i] == "LineMarking") {
              this.slectedFile_arr.push("Line Marking");
              this.sortArrayOFItems.push("LineMarking");
              this.panelDiv = true;
            }
          }
          console.log(this.slectedFile_arr);
  ...
  PanelNonChange(ownerable_id, event) {
    ...
    this.categoryService
      .PanelNonChange(ownerable_id, this.project_is_accept, panel)
      .subscribe(
        (res) => {
          res = res;
          this.loaderService.display(false);
  ...
  downloadBoq(boqId) {
    this.loaderService.display(true);
    this.financeService
      .downloadBoqCheatSheet(this.project_is_accept, boqId)
      .subscribe(
        (res) => {
          this.loaderService.display(false);
  ...
  PanelNonChangeForThreeD(i_id, event) {
    ...
    this.categoryService
      .PanelNonChangeForThreeD(i_id, this.project_is_accept, panel)
      .subscribe(
        (res) => {
          res = res;
          this.loaderService.display(false);
  ...
  viewChild(obj) {
    ...
    this.loaderService.display(true);
    this.categoryService
      .getViewChild(this.project_is_accept, this.handover_id)
      .subscribe(
        (res) => {
          //

          this.loaderService.display(false);
          this.child_file_type = Object.keys(
            res["project_handover"]["child_details"]
          );
          this.child_list =
            res["project_handover"]["child_details"][this.child_file_type[0]];
        },
        (err) => {
          this.loaderService.display(false);
        }
  ...
  fileUpload(event, label) {
    ...
    for (let file of event.target.files) {
      ...
      fileReader.onload = (fileLoadedEvent) => {
        ...
        if (event.target.files.length > 1) {
          ...
          this.currentQCFileUploaded.push({
            attachment: base64.result.replace(
      ...
      document.getElementById(label).innerHTML =
        event.target.value.replace("C:\\fakepath\\", "").substring(0, 20) +
        "...";
    }
    if (event.target.files.length > 1) {
      document.getElementById(label).innerHTML =
        event.target.files.length + " Files Uploaded !";
    }
  }
  ...
  updateQC(qcType) {
    ...
    if (
      this.currentQCFileUploaded == undefined ||
      (this.currentQCFileUploaded &&
        this.currentQCFileUploaded.attachment == "")
    ) {
      this.loaderService.display(false);
      this.erroralert = true;
        ...
        .subscribe(
          (res) => {
            ...
            if (qcType == "design_qc" && this.currentQCStatus) {
              this.successMessage =
                "File Uploaded Successfully. If Line Marking is not completed, Please initiate the Line Marking Process";
            } else {
  ...
  updateDepartment() {
    this.loaderService.display(true);
    if (this.curentdep == this.existeddep) {
      alert("You canot assign Same Department");
      this.loaderService.display(false);
    } else {
      this.leadService
        .updateDepartment(
          this.project_id,
          this.projectHandoverId,
          this.curentdep
        )
        .subscribe(
          (res) => {
  ...
  fetchUrl(id){
    ...
    this.leadService.FetchUrl(id).subscribe(
      (res) => {
        ...
        //  window.open(res.csv_download_url,'_self' )
        // setTimeout(
        //   function () {
        //     this.successalert = false;
        //   }.bind(this),
