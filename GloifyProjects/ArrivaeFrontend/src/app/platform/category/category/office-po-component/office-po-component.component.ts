...
@Component({
  selector: "app-office-po-component",
  templateUrl: "./office-po-component.component.html",
  styleUrls: ["./office-po-component.component.css"],
  providers: [CategoryService, LeadService],
})
export class OfficePoComponentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.milestoneForm = this.formBuilder.group({
      ...
      type: new FormControl(this.project_type),
  ...
  CreatAddsliForm() {
    this.AddSliForm = this.formBuilder.group({
      ...
      sli_type: new FormControl(this.project_type),
    });
  }
  getProcurementList2() {
    this.automated_sli = true;
  ...
  updateSlis2() {
    this.loaderService.display(true);
    this.categoryService
      .updateSliItemsbulk(this.Sli_id, this.update_Sli_Form2.value)
      .subscribe(
        (res) => {
          this.successalert = true;
          this.loaderService.display(false);
  ...
  poSubmission() {
    ...
    console.log(this.milestoneForm.controls);
    this.getTotalAmount();
  }
